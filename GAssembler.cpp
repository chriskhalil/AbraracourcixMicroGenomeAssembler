...
struct Paramerters{
    ...
    const int extend_penalty;

    Paramerters(const int mismatches,const unsigned int threads = 1, const unsigned int match = 1, const unsigned int mismatch = 4, const unsigned int gap = 6, const unsigned int extend = 1):
    allowed_mismatches(mismatches),threads(threads),match_score(match),mismatch_penalty(mismatch),gap_penalty(gap),extend_penalty(extend){}
};
...
void displayUsage() {
    std::cout << "Usage: abraracourcix [-ms <match_score>] [-mp <mismatch_penalty>] [-gp <gap_penalty>] [-ep <extended_penalty>] [-vb] [-cr] [-mas] -irf <input_reference> -ird <input_reads> -m <allowed_mismatches> -t <num_threads>\n";
}

void PrintMascot();
int main(int argc, char *argv[]) {

    //TODO: noinput validationg will be made as in if user enters a char rather than num.
    
    // Define default values for optional arguments
    int defaultMatchScore = 0;
    int defaultMismatchPenalty = 2;
    int defaultGapPenalty = 1;
    int defaultExtendedPenalty = 3;
    ...
    p.run(reference,reads,Paramerters(stoi(args["-m"]),stoi(args["-t"])));
